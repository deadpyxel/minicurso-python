...
def crawl_results(start_url):
    ...
    for i in range(1, int(page_id) + 1):
        ...
        # Detalhes
        details_box = soup.find_all('p',
                                    attrs={'class': 'text detail-specific'})
        # categoria
        category_box = soup.find_all('p',
                                     attrs={'class': 'text detail-category'})
